...
class UserKbParameters(BaseKbParameters):
    def __init__(self, fields_to_validate: list[str]):
        self.parameters: dict[str, BaseKbParameter] = dict(
            ...
            top_p=NumKbParameter(min=0, max=1, step=0.05, type=float),
            top_k=NumKbParameter(min=1, max=50, step=1, type=int),
            repeat_penalty=NumKbParameter(min=0, max=2, step=0.1, type=float),
            max_tokens=NumKbParameter(min=0, max=32000, step=512, type=int),
...
class VoiceKbParameters(BaseKbParameters):
    def __init__(self, fields_to_validate: list[str]):
        self.parameters: dict[str, BaseKbParameter] = dict(
            voice_name=CheckBoxKbParameter(
                choices=list(Config.VOICE_NAME_TO_IDX),
                value=list(Config.VOICE_NAME_TO_IDX)[-1],
            ),
        )
        super().__init__(fields_to_validate)
